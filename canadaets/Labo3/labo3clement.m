...
for j = 1:length(WV)
    ...
    plot(TAS,GS_sans_vent(:,j),'green'); hold on
    plot(TAS,GS_vent_dos(:,j),'red');
...
for k=1:length(TAS)
	for j=1:length(WV)
        ...
        for i=1:length(WA)
            ...
            GS(i,j,k) = rad2deg(TAS(k)*sin(deg2rad(WA(i)-WCA(i,j,k)))/sin(deg2rad(WA(i))));
        end
	end
    figure();
    plot(WD, WCA(:,:,k))
    titre = ['Graphique de l''angle de la déerive pour une vitesse TAS = ' num2str(TAS(k))];
    xlabel('WD (deg)');
    ylabel('WCA (deg)');
    title(titre);
    legend(M)
